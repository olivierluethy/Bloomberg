    ...
    <Panel
      ...
      actions={
        <button
          ...
          className="border border-line px-1.5 py-0.5 font-mono text-2xs text-fg-dim transition-colors hover:border-line-bright hover:text-fg"
        >
          EDIT
        </button>
      }
            ...
            <div role="tablist" aria-label="Watchlists" className="flex shrink-0 overflow-x-auto border-b border-line">
              {lists.map((list) => (
                <button
                  ...
                  className={cn(
                    "shrink-0 border-b-2 px-2.5 py-1.5 text-sm transition-colors",
                    list.id === active?.id
          ...
          {active && active.symbols.length > 0 ? (
            ...
            <div className="flex flex-col items-center gap-2 px-3 py-10 text-center">
              ...
              <button
                type="button"
                onClick={openPalette}
                className="border border-line px-2 py-1 font-mono text-2xs text-amber transition-colors hover:border-amber/60"
              >
                ⌘K — FIND SYMBOLS
              </button>
            </div>
