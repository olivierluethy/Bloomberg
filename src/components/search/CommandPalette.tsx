...
function PaletteBody({ onClose }: { onClose: () => void }) {
  ...
  return (
    <>
      <div className="flex items-center gap-2 border-b border-line px-1.5 py-0.5">
        ...
        <input
          ...
          className="flex-1 bg-transparent text-xs text-amber outline-none placeholder:text-fg-dim"
          role="combobox"
          aria-expanded
        ...
        />
        <kbd className="hidden border border-line px-1 text-2xs text-fg-dim sm:inline">
          ESC
        </kbd>
      </div>
      ...
      <div ref={listRef} id="palette-list" role="listbox" aria-label="Results" className="min-h-0 flex-1 overflow-y-auto py-1">
        {ranked.length === 0 ? (
          <p className="px-1.5 py-3 text-center text-xs text-fg-dim">
            No matches for <span className="text-amber">{query}</span>
          </p>
        ) : (
          groups.map((group) => (
            <div key={group.group} className="mb-1">
              <div className="eyebrow px-1.5 py-0.5">{group.group}</div>
...
function Row({
  ...
  return (
    <div
      ...
      className={cn(
        "flex cursor-pointer items-center gap-1.5 px-1.5 py-0.5 transition-colors",
        active ? "bg-amber text-black" : "text-amber",
      )}
    >
      {item.group === "Symbols" && (
        <span
          className={cn("w-3 shrink-0 text-center text-xs", active ? "text-black" : item.pinned ? "text-amber2" : "text-fg-dim")}
          aria-hidden
        >
          {item.pinned ? "★" : "☆"}
        </span>
      ...
      <span className="shrink-0 text-xs font-bold">
        <Highlight text={item.title} indices={indices} active={active} />
      </span>
      {item.subtitle && (
        <span className={cn("min-w-0 flex-1 truncate text-xs", active ? "text-black" : "text-fg-dim")}>
          {item.subtitle}
        </span>
      )}
      {item.tag && (
        <span className={cn("ml-auto shrink-0 text-2xs font-bold", active ? "text-black" : "text-fg-dim")}>{item.tag}</span>
      )}
    </div>
  );
}
...
/**
 * Renders matched characters so the ranking is legible.
 *
 * The active row is inverted (black on amber), so it needs its own treatment:
 * highlighting in amber2 there paints amber-on-amber and the matched characters
 * vanish — the exact letters you just typed become the only invisible ones. On
 * an active row the match is carried by the underline against inherited black.
 */
  ...
  return (
    <>
      {text.split("").map((char, i) => (
        <span key={i} className={set.has(i) ? (active ? "underline" : "text-amber2 underline") : undefined}>
          {char}
        </span>
      ))}
    </>
...
function Footer({ activeItem, activeListName }: { activeItem?: PaletteItem; activeListName?: string }) {
  ...
  return (
    <div className="flex shrink-0 items-center gap-1.5 border-t border-line px-1.5 py-0.5 text-2xs text-fg-dim">
      ...
      <span className="ml-auto font-bold tracking-wide text-amber2">TERM</span>
    </div>
  );
}
...
function Hint({ keys, label }: { keys: string; label: string }) {
  return (
    <span className="flex items-center gap-1.5">
      <kbd className="border border-line px-1 text-2xs text-fg-dim">{keys}</kbd>
